...
`r max_steps_value` on the `r max_steps_interval`th interval.
...
We can see that there are `r sum(is.na(raw_data$steps))` missing values out of 
`r nrow(raw_data)` rows, which makes up for 
`r 100*sum(is.na(raw_data$steps)) / nrow(raw_data)`% of the whole dataset, 
